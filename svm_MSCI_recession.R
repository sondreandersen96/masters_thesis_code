...
# Importing data
df_est <- read.csv('equal_datasets/swe_est_w_index.csv')
df_test <- read.csv('equal_datasets/swe_test_w_index.csv')
...
n_recession_est <- sum(df_est$recession)
n_recession_test <- sum(df_test$recession)

# Lager en ny dataframe med bare variabler som skal være med i analysen
# .. og konverterer output variablen til faktorer
df_estimation <- data.frame(recession=as.factor(df_est$recession),
                            stock_return_L = df_est$stock_return_L12)
df_estimation <- na.omit(df_estimation)

df_testing <- data.frame(recession=as.factor(df_test$recession),
...
tune.out=tune(svm, recession~., data=df_estimation, kernel="radial",
...
for (i in cost_intervals){
  svmfit <- svm(recession~., data=df_estimation, kernel="radial", cost=i, gamma=0.01, scale=FALSE)
  ypred_insample = predict(svmfit, df_estimation)
  t <- table(predict=ypred_insample, thruth=df_estimation$recession)
  ...
  bias <- round((tp + fp) / n_recession_est, digits=2)
...
df_roc <- df_roc[-c(1),]
# Export results
write.csv(df_roc,"1_analysis/stock_data_analysis/recession_svm/insample_nor3.csv", row.names = FALSE)
#write.csv(df_roc,"1_analysis/output_forecasting_svm/insample_12_i_nor_.csv", row.names = FALSE)

# ---> Out of sample test 
svmfit <- svm(recession~., data=df_estimation, kernel="radial", cost=1, gamma=0.01, scale=FALSE)
plot(svmfit, df_estimation) # Insample plot

ypred = predict(svmfit, df_testing)
table(predict=ypred, thruth=df_testing$recession)
